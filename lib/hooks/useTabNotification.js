...
export function useTabNotification(
    ...
    const [prependTitle, setPrependTitle] = useState('')
    ...
    useEffect(() => {
        if (prependTitle) {
            setModifiedTitle(prependTitle + ' ' + originalTitle)
        } else {
            setModifiedTitle(originalTitle)
        }
    }, [prependTitle, originalTitle])
    ...
    return {setPrependTitle, setFlashMessage, isShown, setIsShown};
}
